...
public class CloudantTest {
  ...
  // Test the postPartitionExplainAsStream operation with a valid options model parameter
  @Test
  public void testPostPartitionExplainAsStreamWOptions() throws Throwable {
    // Register a mock response
    String mockResponseBody = "{\"foo\": \"this is a mock response for JSON streaming\"}";
    String postPartitionExplainAsStreamPath = "/testString/_partition/testString/_explain";
    server.enqueue(new MockResponse()
      .setHeader("Content-type", "application/json")
      .setResponseCode(200)
      .setBody(mockResponseBody));
    ...
    // Invoke postPartitionExplainAsStream() with a valid options model and verify the result
    Response<InputStream> response = cloudantService.postPartitionExplainAsStream(postPartitionExplainOptionsModel).execute();
    assertNotNull(response);
    InputStream responseObj = response.getResult();
    assertNotNull(responseObj);
    ...
    // Verify request path
    String parsedPath = TestUtilities.parseReqPath(request);
    assertEquals(parsedPath, postPartitionExplainAsStreamPath);
    // Verify that there is no query string
    Map<String, String> query = TestUtilities.parseQueryString(request);
    assertNull(query);
    // Verify streamed JSON response
  ...
  // Test the postPartitionExplainAsStream operation with and without retries enabled
  @Test
  public void testPostPartitionExplainAsStreamWRetries() throws Throwable {
    cloudantService.enableRetries(4, 30);
    testPostPartitionExplainAsStreamWOptions();

    cloudantService.disableRetries();
    testPostPartitionExplainAsStreamWOptions();
  }

  // Test the postPartitionExplainAsStream operation with a null options model (negative test)
  @Test(expectedExceptions = IllegalArgumentException.class)
  public void testPostPartitionExplainAsStreamNoOptions() throws Throwable {
    server.enqueue(new MockResponse());
    cloudantService.postPartitionExplainAsStream(null).execute();
  }
