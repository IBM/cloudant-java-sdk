...
public class CloudantIT extends SdkIntegrationTestBase {
  ...
  public void testPostPartitionExplainAsStream() throws Exception {
    try {
      PostPartitionExplainOptions postPartitionExplainOptions = new PostPartitionExplainOptions.Builder()
        .db("testString")
        .partitionKey("testString")
      ...
      // Invoke operation
      Response<InputStream> response = service.postPartitionExplainAsStream(postPartitionExplainOptions).execute();
      // Validate response
      assertNotNull(response);
      assertEquals(response.getStatusCode(), 200);
  ...
  @Test(dependsOnMethods = { "testPostPartitionExplainAsStream" })
